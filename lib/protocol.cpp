...
ProtocolString::ProtocolString(const ProtocolString &other)
    : length(other.length) {
...
// (Think about the destructor: which object frees the shared array first?)
 ...
 *
 * Converts Message objects to/from binary wire format for network transmission
 * or storing data into disc. Uses little-endian byte order and assumes same
 * architecture on both ends. No compression or encryption - focused on
 * demonstrating memory safety issues.
 */
...
// NOTE: Assumes the payload pointer matching header.type is non-null
// (enforced by Message's constructor) - no defensive checks are made here.
...
std::vector<uint8_t> Serializer::serialize(const Message &msg) {
  ...
  case USER_INFO: {
    ...
    // Write basic fields
    buffer.resize(offset + sizeof(uint32_t) + sizeof(uint16_t) +
                  sizeof(uint16_t));
    buffer.resize(offset + sizeof(uint32_t) * 2 + sizeof(uint16_t));
    memcpy(buffer.data() + offset, &user.user_id, sizeof(uint32_t));
    offset += sizeof(uint32_t);
    memcpy(buffer.data() + offset, &user.status, sizeof(uint16_t));
    offset += sizeof(uint16_t);
    memcpy(buffer.data() + offset, &user.tag_count, sizeof(uint32_t));
    offset += sizeof(uint32_t);

    // Calculate total size needed for strings
    size_t string_size = sizeof(uint16_t) + user.username.length +
                         sizeof(uint16_t) + user.email.length;
    for (uint32_t i = 0; i < user.tag_count; i++) {
  ...
  case FILE_CHUNK: {
    ...
    size_t total_size = sizeof(uint32_t) * 2 + sizeof(uint16_t) +
                        filename_size + chunk.chunk_size;
...
// INTENTIONAL BUGS: Is all input validated? Check:
// - Is the magic number or protocol version ever checked?
// - Are unknown header types (values 0, 4, 255, ...) handled?
...
Message *Serializer::deserialize(const uint8_t *data, size_t length) {
  ...
  case USER_INFO: {
    ...
    offset += sizeof(uint16_t);
    memcpy(&msg->user_info->tag_count, data + offset, sizeof(uint32_t));
    offset += sizeof(uint32_t);

    offset +=
        read_string(data + offset, length - offset, msg->user_info->username);
    offset +=
        read_string(data + offset, length - offset, msg->user_info->email);

    // Bug: tag_count comes straight from untrusted input and is not checked
...
// INTENTIONAL BUG: The caller pre-sizes the buffer (see serialize), but
// write_string itself performs no checks - if a size computation upstream
// is wrong, this writes past the end of the buffer.
...
// INTENTIONAL BUG: The length prefix is trusted without checking that
// `sizeof(uint16_t) + str.length` fits within `remaining` - a crafted
// length field can cause reads past the end of the buffer.
// Returns number of bytes consumed, or 0 on error
size_t Serializer::read_string(const uint8_t *buffer, size_t remaining,
                               ProtocolString &str) {
